...
def split():
    ...
    dataset_path = str(os.getcwd())+"/Users_slab/" + \
        current_loggin_user+"/"+gLabel
...
def move_modelfile():
    ...
    target_dir = str(getcwd()+"/Users_slab/" +
                     current_loggin_user+'/'+gLabel+'/model')
    ...
    source_dir1 = str(getcwd()+"/Users_slab/"+current_loggin_user +
                      '/'+gLabel+'/model/weights/'+gLabel+'.pt')
    ...
    add_model = User_Models_record(
        username=current_loggin_user, user_id=current_loggin_userid, model_name=gLabel, generated_date=genrated_on)
...
def rename_modelfile():
    source_filepath = (
        str(os.getcwd())+"/yolov5/runs/train/"+gLabel+"/weights/best.pt")
    rename_filepath = (str(os.getcwd())+"/yolov5/runs/train/" +
                       gLabel+"/weights/"+gLabel+".pt")
...
def download():
    Path = (model_path+"/Users_slab/" +
            current_loggin_user+"/Models/"+gLabel+'.pt')
...
class Objdetection():
    def __init__(self, url):
        self.video = cv2.VideoCapture(
            "rtmp://media5.ambicam.com:1938/live/3eb6f300-bb4a-4f66-af0c-15290833e3de")
        self.url = url
        self.error_count = 0
        ...
        self.model = torch.hub.load('yolov5', 'custom', path=loadModel + '/yolov5/runs/train/' +
                                    gLabel+'/weights/'+gLabel+'.pt', source='local', force_reload=True)

    def __del__(self):
        self.video.release()

    def get_frame(self):
        path_model = loadModel + "/yolov5/train/" + \
            gLabel + "/" + "weights" + "/" + gLabel+'.pt'
...
@app.route("/BWphotos")
def bwPhotos():
    binderList = os.listdir(
        "/home/torquehq/torquehq-io/Github/Torque-AI/Users_slab/test/a1")
...
class FR_candidate_addon(object):
    ...
    def get_frame(self):
        ...
        if success:
            ...
            print(str(count) + " images taken and saved to " +
                  fn_name + " folder in database ")
...
class Person_Counter(object):
    ...
    def start_record(self):
        ...
        self.recordingThread = RecordingThread(
            "Video Recording Thread", self.vs)
...
class Fire_detection():
    def __init__(self, url):
        ...
        self.model = torch.hub.load(
            'yolov5', 'custom', path='Fire_detection/fire.pt', source='local', force_reload=True)
...
def update():
    if request.method == 'POST':
        ...
        current_loggin_user = current_user.username
        admin = User_camera_sources.query.filter_by(
            username=current_loggin_user).first()
        admin.link1 = link1
        admin.link2 = link2
...
def delete(id):
    ...
    model_to_delete_file = User_Models_record.query.filter_by(
        serial_no=id).first()
    model_to_delete_local = model_to_delete_file.model_name
    print(model_to_delete)
    os.remove(os.path.join(str(os.getcwd()+"/Users_slab/" +
              current_loggin_user+"/Models/"), model_to_delete_local+".pt"))
...
def rename_modelname(id):
    ...
    if request.method == 'POST':
        ...
        rename_to_model_filename_path = (os.path.join(str(os.getcwd(
        )+"/Users_slab/"+current_loggin_user+"/Models/"), model_to_rename.model_name + ".pt"))
        renamed_model_path = (os.path.join(str(os.getcwd(
        )+"/Users_slab/"+current_loggin_user+"/Models/"), new_model_name + ".pt"))
        renamed_file = os.rename(
            rename_to_model_filename_path, renamed_model_path)
        ...
        with open(f.filename, 'r') as csv_file:
            ...
            for row in listformate_csvfile:
                if (len(row) != 0):
                    ...
                    name_source = row[1]
                    add_model = User_camera_sources_record(
                        username=current_loggin_user, user_id=current_loggin_userid, source=source, name_source=name_source)
                    db.session.add(add_model)

                    db.session.commit()

                    print("SOURCE :  {} , CAMERA_NAME: {} is successfully added in database ! ".format(
                        source, name_source))
...
def insert_src():
    if request.method == "POST":
        ...
        current_loggin_user = current_user.username
        add_model = User_camera_sources_record(
            username=current_loggin_user, user_id=current_loggin_userid, source=source_link, name_source=name_of_camera)
        db.session.add(add_model)
        db.session.commit()
...
def delete_src(id):
    ...
    source_to_delete_file = User_camera_sources_record.query.filter_by(
        serial_no=id).first()
...
class Objdetection_try():
    ...
    def __init__(self, url):
        ...
        self.error_count = 0
        self.model = torch.hub.load(
            'yolov5', 'custom', path='/home/torque/Desktop/main/torqueai/yolov5s.pt', source='local', force_reload=True)
...
class pre_selected_model(db.Model):
    __tablename__ = 'pre_selected_model'

    user_id = db.Column('user_id', db.Integer,
                       primary_key=True)
    username = db.Column(db.String(1000))
    pre_selected_model = db.Column(db.String(3000))
    pre_selected_model_scr2 = db.Column(db.String(3000))

    def __init__(self, user_id, username, pre_selected_model, pre_selected_model_scr2):

        self.user_id = user_id
        self.username = username
        self.pre_selected_model = pre_selected_model
        self.pre_selected_model_scr2 = pre_selected_model_scr2
...
class Objdetection_4multi():
    ...
    def get_frame(self):
        ...
        if success == True:
            ...
            # object.print()  # print results to screen
            objects = pred.xyxy[0]
            labels = self.model.module.names if hasattr(
                self.model, 'module') else self.model.names
            for i in range(len(objects)):
                ...
                if obj[5] in keys:
                    ...
                    print(label)
                    cv2.rectangle(frame, (int(obj[0]), int(obj[1])), (int(
                        obj[2]), int(obj[3])), (0, 255, 0), 2)
                    cv2.putText(frame, label, (int(obj[0]), int(
                        obj[1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
class Objdetection_multifeed():
    ...
    def __init__(self, url):
        ...
        user = pre_selected_model.query.filter_by(
        username=current_loggin_user ).first()
        if user.pre_selected_model != None:

            self.model = torch.hub.load('yolov5', 'custom', path=(str(os.getcwd())+"/Users_slab/"+current_loggin_user+"/Models/"+user.pre_selected_model +".pt"), source='local', force_reload=True)
        else:
        ...
        print("Screen 1 model :", user.pre_selected_model )
...
Roww = ''
@app.route('/differ_model', methods=['POST', 'GET'])
def differ_model():
    current_loggin_user = current_user.username
    # current_loggin_userid = current_user.get_id()
    user = pre_selected_model.query.filter_by(
            username=current_loggin_user ).first()
    # if user != current_loggin_userid or user == None:
    #             add_model = pre_selected_model(username=current_loggin_user, user_id=current_loggin_userid,
    #                                         pre_selected_model='yolov5s', pre_selected_model_scr2='yolov5s')
    #             db.session.add(add_model)

    #             db.session.commit()
    return render_template('home/multifeed.html', User_Models_record=User_Models_record.query.filter_by(username=current_user.username), User_camera_sources=User_camera_sources_record.query.filter_by(username=current_user.username),screen1_model = user.pre_selected_model,screen2_model=user.pre_selected_model_scr2)


@app.route('/model', methods=['POST', 'GET'])
def model():
    global Roww, cknamess
    if request.method == "POST":
        ...
        if cknamess:
            user.pre_selected_model = cknamess
            db.session.commit()
       
        print("Data Updated Successfully")
        
     
    return redirect(url_for('differ_model'))
# ------------------------ feed 2 ------------------------------------#
...
def model2():
    ...
    if request.method == "POST":
        ...
        if cknames2:
            user.pre_selected_model_scr2 = cknames2
...
class Objdetection_multifeed2():
    ...
    def __init__(self, url):
        ...
        if user.pre_selected_model_scr2 != None:
            self.model = torch.hub.load('yolov5', 'custom', path=(str(os.getcwd())+"/Users_slab/"+current_loggin_user+"/Models/"+user.pre_selected_model_scr2 +".pt"), source='local', force_reload=True)
        else:
             self.model = torch.hub.load('yolov5', 'custom', path=(str(os.getcwd())+"/Users_slab/"+current_loggin_user+"/Models/"+"yolov5s.pt"), source='local', force_reload=True)
        print("SCREEN- 2: ", user.pre_selected_model_scr2 )
        print(self.url)
